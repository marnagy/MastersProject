...
def get_args() -> Namespace:
    ...
    parser.add_argument('--index-col', help='If CSV has index column, please include its name here.')
...
def main() -> None:
        ...
        if df.dtypes[column_name] == object: # categorical
            if args.one_hot:
                ...
                for col_i, col in enumerate(one_hot_encoded_column.columns):
                    ...
                    df.insert(column_index + col_i, new_column_name, one_hot_encoded_column[col])
                    df[new_column_name] = df[new_column_name].astype(np.int64)
                column_index += len(one_hot_encoded_column.columns)
                last_column_converted_to_columns_amount = len(one_hot_encoded_column.columns)
            else:
                values: List[str] = df[column_name].unique().tolist()
                df.insert(column_index, f'{column_name}-converted', df[column_name].apply(lambda val: values.index(val)))
                df.drop(columns=[column_name], inplace=True)
                column_index += 1
                last_column_converted_to_columns_amount = 1
        else:
    ...
    print(df.head(5))
    print(f'Input columns: {len(df.columns) - last_column_converted_to_columns_amount}, output_columns: {last_column_converted_to_columns_amount}')

    df.to_csv(f'prepared_{args.filepath}', sep=args.delimiter, index=args.include_index)
